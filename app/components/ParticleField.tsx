...
export default function ParticleField() {
  ...
  useEffect(() => {
    ...
    const HUB_COUNT         = isMobile ?  2 :  5;   // Large "neural hub" nodes
    const CONNECTION_DIST   = isMobile ? 60 : 100;  // Shorter on mobile for perf
    const MOUSE_RADIUS      = isMobile ?  0 : 160;  // No hover on touch
    ...
    function createParticles() {
      ...
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        particles.push({
          ...
          baseSize:    isHub ? 1.6 + Math.random() * 0.6 : 0.4 + Math.random() * 0.5,
          baseOpacity: isHub ? 0.45 + Math.random() * 0.2 : 0.08 + Math.random() * 0.2,
          isHub,
    ...
    function draw() {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        const opacity      = p.baseOpacity * depthFactor * (nearMouse ? 1.2 : 0.4);

        /* ── Hub pulse ring — drawn BEFORE particle so it's behind ── */
        if (p.isHub) {
          const ringR   = size + p.pulse * 20;
          const ringAlp = (1 - p.pulse) * 0.05 * depthFactor;
          ctx!.beginPath();
          ctx!.arc(p.x, p.y, ringR, 0, Math.PI * 2);
          ctx!.strokeStyle = `rgba(172,0,87,${ringAlp})`;
          ctx!.lineWidth   = 0.8;
          ctx!.stroke();
        }
        ...
        if (nearMouse || p.isHub) {
          ctx!.fillStyle = p.isHub ? `rgba(172,0,87,${opacity})` : `rgba(252,74,31,${opacity})`;
        } else {
        ...
        if ((p.isHub || nearMouse) && size > 0) {
          ...
          const rColor = p.isHub ? 172 : 252;
          const gColor = p.isHub ? 0 : 74;
          const bColor = p.isHub ? 87 : 31;
          const grd = ctx!.createRadialGradient(p.x, p.y, size * 0.4, p.x, p.y, size * 3.5);
          grd.addColorStop(0, `rgba(${rColor},${gColor},${bColor},${opacity * 0.15})`);
          grd.addColorStop(1, `rgba(${rColor},${gColor},${bColor},0)`);
          ctx!.fillStyle = grd;
          ctx!.fill();
        }
        ...
        if (CONNECTION_DIST > 0) {
          for (let j = i + 1; j < particles.length; j++) {
            ...
            const lineAlp = proximity * 0.04 * avgDepth * (nearEither ? 2.0 : 1) * (isHubEdge ? 1.3 : 1);

            ctx!.beginPath();
            ctx!.moveTo(p.x, p.y);
            ctx!.lineTo(q.x, q.y);
            const rColor = isHubEdge ? 172 : 252;
            const gColor = isHubEdge ? 0 : 74;
            const bColor = isHubEdge ? 87 : 31;
            ctx!.strokeStyle = `rgba(${rColor},${gColor},${bColor},${lineAlp})`;
