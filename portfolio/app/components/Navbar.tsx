...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const sections = hashLinks.map((link) =>
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-16">
          {/* Logo */}
          <a href={isHome ? "#" : "/"} className="relative z-50 group flex items-center gap-1">
            <span className="text-xl font-black text-primary tracking-tight transition-colors duration-300 group-hover:text-accent">
              
            </span>
            <span className="text-xl font-black text-accent">HK.</span>
          ...
          <ul className="hidden md:flex items-center gap-1">
            {navLinks.map(({ label, href }) => {
              ...
              const isActive = isHash ? (isHome && activeSection === href) : (pathname === href);
              return (
                <li key={href}>
                  <a
                    href={linkTarget}
          ...
          >
            <motion.span
              animate={
                mobileOpen
                  ? { rotate: 45, y: 10, backgroundColor: "#fc4a1f" }
              ...
              {navLinks.map(({ label, href }, i) => {
                ...
                return (
                  <motion.a
                    ...
                    className={`group flex items-center gap-4 ${isActive ? "text-accent" : ""}`}
                  >
                    <span className="text-xs font-mono text-accent/50 w-6">
                      0{i + 1}
                    </span>
                    <span className={`text-[clamp(1.25rem,7vw,3.5rem)] font-black uppercase tracking-tight transition-colors duration-200 ${
